...
static uint16 genCMSG_SWAP_ITEM(lua_State* L, byte* buf) {
	GL_START;
	M(byte, dstbag);
	M(byte, dstslot);
	M(byte, srcbag);
...
static uint16 genCMSG_OPEN_ITEM(lua_State* L, byte* buf) {
	GL_START;
	M(byte, bagSlot);
	M(byte, slot);
	GL_END;
...
static uint16 genCMSG_BUY_BANK_SLOT(lua_State* L, byte* buf) {
	GL_START;
	M(Guid, guid);
	GL_END;
}

static uint16 genCMSG_QUEST_CONFIRM_ACCEPT(lua_State* L, byte* buf) {
	GL_START;
	M(uint32, questId);
	GL_END;
}

static uint16 genCMSG_SET_AMMO(lua_State* L, byte* buf) {
	GL_START;
	M(uint32, itemId);
	GL_END;
}
...
static uint16 genCMSG_AREATRIGGER(lua_State* L, byte* buf) {
	GL_START;
	M(uint32, triggerID);
	GL_END;
}
...
#define GEN_CASE(name) case name: return gen##name;
	switch(opcode) {
		...
		GEN_CASE(CMSG_SWAP_ITEM);
		GEN_CASE(CMSG_OPEN_ITEM);
